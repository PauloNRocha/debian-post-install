...
echo "║               Instalador de Aplicações Essenciais            ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo -e "${YELLOW}Use: sudo ./04-essential-apps.sh${NC}"
    exit 1
fi
...
ESSENTIAL_PACKAGES="git curl wget vim build-essential"

print_info "Pacotes a serem instalados: ${CYAN}${ESSENTIAL_PACKAGES}${NC}"

if ! run_with_progress "Instalando aplicações essenciais" "apt install -y $ESSENTIAL_PACKAGES"; then
    print_error "A instalação das aplicações essenciais falhou."
...
echo -e "🎉 ${GREEN}${BOLD}Aplicações essenciais foram instaladas!${NC}"
echo ""
echo -e "Ferramentas como git, curl, vim e compiladores C/C++ já estão disponíveis."
echo ""
